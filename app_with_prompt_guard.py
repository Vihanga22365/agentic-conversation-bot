...
@tool("Save Human message in memory")
def memory_manege(user_answer: str) -> str:
    """Save Human message in memory"""
...
configJail = RailsConfig.from_path('./configJail.yml')
guardrails = RunnableRails(config)
guardrailsJail = RunnableRails(configJail)
...
def ask_rag_question(question: str) -> str:
    ...
    response = rag_chain_with_guardrails.invoke(question)
    return response
...
#     if human_response:
#       result = check_moderation_flag(human_response["output"])
#       if(result):
#         run_sync(cl.AskUserMessage(content=f"We're sorry, but your input has been as inappropriate.", timeout=1).send())
#         return "We're sorry, but your input has been as inappropriate."
#       else:
...
#     human_response = run_sync(cl.AskUserMessage(content=f"{question}", timeout=600).send())
#     if human_response:
#       chat_msg = [{"role": "user", "content": human_response["output"]}]
#       formatted_prompt = moderation_prompt_for_chat(chat_msg)
...
def ask_human(question: str) -> str:
    ...
    if human_response:
      ...
      chat_msg = human_response["output"]
      ...
      result = chain_with_guardrails.invoke({"input": chat_msg})
      
      print("Result: ", result) 
      
      
      if ('output' in result and result['output'] == "I'm sorry, I can't respond to that.") or result == "I'm sorry, I can't respond to that." or result == "I'm really sorry, but I can't assist with that.":
        print("===========================================")
        print(result)
        print("===========================================")
        run_sync(cl.AskUserMessage(content=f"Thank you for your input! To ensure a safe and positive experience for everyone, we have some guidelines in place. If you have any questions or need assistance on a different topic, please proceed.", timeout=1).send())
        return "Thank you for your input! To ensure a safe and positive experience for everyone, we have some guidelines in place. If you have any questions or need assistance on a different topic, please proceed."
      else:
        chat_msg = human_response["output"]
        # prompt = ChatPromptTemplate.from_messages([
        #     ("user", "{input}")
        ...
        # chain_with_guardrails = guardrailsJail | chain
        
        # result = chain_with_guardrails.invoke({"input": chat_msg})
        score = get_jailbreak_score(chat_msg)
...
Ensure the output matches exactly the specified formats above depending on whether the task is executed successfully or if the content is irrelevant.
  """,
  agent=dispute_help_agent,
  tools=[ask_human, get_dispute_tool, memory_manege],
  verbose=True,
  callback=callback_function,
)
...
Ensure the output matches exactly the specified formats above depending on whether the task is executed successfully or if the content is irrelevant.
 """,
  agent=dispute_help_agent,
  tools=[ask_human, update_dispute_status_tool, memory_manege],
  verbose=True,
  callback=callback_function,
)
...
8. **Goal:**
  ...
  tools=[ask_human, send_messages_tool, memory_manege],
  verbose=True,
  callback=callback_function,
)
...
@cl.on_chat_start
def start_chat():
    items_crew.kickoff(inputs={'user_details': user_details, 'chat_history': memory})
